...
export default function HorizontalScrollGallery() {
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-auto lg:h-[100vh] py-20 lg:py-0 bg-background overflow-hidden lg:overflow-hidden flex flex-col lg:justify-center border-t border-border mt-10 md:mt-0" style={{ perspective: "2000px" }}>

      {/* High-End Typography */}
      <div className="intro-block relative lg:absolute lg:left-[10vw] lg:top-[45%] lg:-translate-y-1/2 z-20 max-w-2xl pointer-events-none px-6 lg:px-0 mb-10 lg:mb-0">
        <h2 className="text-5xl md:text-7xl lg:text-9xl font-black tracking-tighter mb-4 text-foreground leading-[0.9] lg:leading-[0.8] drop-shadow-2xl">
          Continual Design <br />
          <span className="text-accent font-serif italic text-4xl md:text-6xl lg:text-8xl drop-shadow-[0_0_20px_rgba(249,115,22,0.6)]">Exploration</span>
        </h2>
        <p className="text-lg md:text-xl lg:text-2xl text-muted-foreground font-bold leading-tight max-w-lg border-l-4 border-accent pl-4 lg:pl-8 py-2">
          To keep my creative edge, I regularly prototype and collect modern industrial inspirations.
        </p>
      </div>

      <div className="absolute top-4 lg:top-12 left-6 lg:left-10 z-10 opacity-60">
      ...
      {/* Grid Container: Clean 2-Row Layout shifted to the right half on desktop */}
      <div
        ref={containerRef}
        className="flex flex-col lg:flex-col lg:flex-wrap lg:content-start lg:h-[88vh] lg:mt-16 lg:pt-[2vh] lg:ml-[45vw] gap-8 lg:gap-x-20 lg:gap-y-14 w-full lg:w-max px-6 lg:px-0"
      ...
      >
        {categories.map((category: any, idx: number) => {
          ...
          return (
            <div
              key={category._id}
              className="horizontal-panel flex shrink-0 w-full lg:w-[48vw] h-[40vh] md:h-[42vh] items-center justify-center transition-opacity"
              style={{ transformStyle: "preserve-3d" }}
            >
